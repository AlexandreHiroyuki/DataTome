...
public:
  ...
  double median() {
    ...
    size_t current_size = this->point_count(), m = current_size / 2;
    TypeOfArray *temp_array =
        (TypeOfArray *)malloc(sizeof(TypeOfArray) * current_size);
...
private:
  TypeOfArray medianOfMedians(int l, int r, TypeOfArray nums[]) {
    int k       = 5;
    size_t size = r - l + 1, medians_size = (size + k - 1) / k;
    TypeOfArray medians[medians_size];
    for(int i = l, m_count = 0; i <= r; i += k, m_count++) {
      int left = i, right = dt_min(i + k, r + 1),
          mid = left + (right - left) / 2;
      qsort(nums + left, right - left, sizeof(TypeOfArray),
            sort_ascend<TypeOfArray>);
  ...
  TypeOfArray quickSelect(int l, int r, int m, TypeOfArray nums[]) {
    int pivot = medianOfMedians(l, r, nums), index = r, p = l;
    for(int i = l; i <= r; i++) {
      if(nums[i] == pivot) {
        index = i;
